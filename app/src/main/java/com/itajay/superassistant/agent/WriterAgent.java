    ...
    public WriterAgent(ChatModel chatModel,
        ...
        this.reactAgent = ReactAgent.builder()
                ...
                .instruction("""
                        ...
                        第二步：**取用精读结果**。调用 analyzePapers(topic)，
                        它会逐篇精读该课题下所有已下载论文，把四维信息落盘到 investigation/{课题方向}/analysis/，
                        并返回一份索引（论文短名 / 来源等级 / 一句话定位 / 文件路径）。
                        - 已分析过的论文会被自动跳过，重复调用是安全的。
                        - 索引只会告诉你"有哪些论文、各自讲了什么"，**不含四维正文**。
                        - 若索引报告有论文失败，那是正文读不到，按 ABSTRACT_ONLY 处理并在文档中标注。
